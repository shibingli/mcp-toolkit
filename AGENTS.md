...
| 9  | gRPC 同步  | gRPC 通信类功能必须同步修改 Proto 文件内容                                                              |
| 10 | API 参数   | 明确定义的或已知固定的 API 参数，必须使用外部的结构体进行传输交互，不允许采用内嵌结构和 Map 传值                                    |
| 11 | API 路由   | API 必须正确定义路由的提交方式：`GET` / `PUT` / `POST` / `DELETE` / `HEAD` / `OPTIONS`                 |
| 12 | API 响应   | API 响应必须明确正确的 HTTP 状态码                                                                   |
| 13 | 结构体声明    | 所有的结构体都必须放在pkg/types，不允许在其他包中声明结构体，不能使用内嵌结构体                                             |
| 14 | 常量声明     | 对于大于一次的多次字内容引用，要采用常量声明，不允许直接使用字面量                                                        |
| 15 | 禁止批量变更   | 禁止使用命令行或脚本对代码文件进行批量修改操作                                                                  |
| 16 | 代码变更     | 完成代码的变更后，要通过 `gofmt` 格式化代码，然后编译后再进行相关测试或功能验证                                             |
| 17 | 结构体预热    | pkg/types下的结构体必须添加到pkg/utils/json/pretouch.go进行JSON预热                                    |
| 18 | 打包构建     | 打包编译时必须添加编译标签：`go build -tags="sonic"`。需要为Go环境配置环境变量： `GOEXPERIMENT="jsonv2,greenteagc"` |
| 19 | 单元测试     | 所有的业务功实现或工具方法必须创建对应的单元测试，覆盖率必须在 100% 以上。打包编译前必须通过所有的单元测试验证，保证所有单元测试100%通过                |
| 21 | 重复功能代码处理 | 在实现新功能前，要先检查当前项目是否已经存在类似的功能代码，要优先复用已有的功能代码，在全局范围内禁止出现重复的功能代码                             |
| 22 | JSON编译码  | 对于JSON数据的编译码，统一采用`pkg/utils/json/json`处理                                                 |
